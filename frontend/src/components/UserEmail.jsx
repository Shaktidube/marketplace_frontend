...
const UserEmail = ({ sWalletAddress }) => {
  ...
  const { mutate: mutateVerifyOtp  , isPending: isPendingVerifyingOtp } = useMutation({
    mutationFn: (payload) => verifyOtp(payload),
    onSuccess: (data) => {
      console.log(" otp response data.data " , data.data);
      const { sWalletAddress, sToken, isVerified, sEmail, sUsername , sUserProfileImage} =
        data.data;
      dispatch(
        setWalletAddress({
          sWalletAddress,
      ...
      <>
        ...
        {countdown > 0 ? (
          <p className="mt-4 text-gray-400">
            Resend OTP in 00:{countdown < 10 ? `0${countdown}` : countdown} seconds
          </p>
        ) : (
