...
import { getContractInstance, getItemFromLocalStorage, handle409Error, showToast } from "../utils/helper";
...
const MintNft = () => {
  ...
  const { mutate: mutateUploadFile, isLoading: isUploading } = useMutation({
    mutationFn: (file) => uploadFile(file, formData.sNftName, formData.nNftDescription, formData.nRoyalty, import.meta.env.VITE_MINT_CONTRACT_ADDRESS),
    onSuccess: (data) => {
  ...
  const validateForm = () => {
    ...
    } else if (formData.sNftName.trim().length < 1 || formData.sNftName.trim().length > 12) {
    ...
    if (!formData.nNftDescription.trim()) {
      ...
      newErrors.nNftDescription = "Description must be less than 500 characters";
    }
  ...
  const mintNft = async (ipfsurl, Royalty) => {
    try {
      ...
      const tx = await mintContract.safeMint(user.sWalletAddress, ipfsurl, parsedRoyalty);
      ...
      if (error.code === "ACTION_REJECTED") {
        ...
        errorMessage = error.reason || error.message || "Contract error occurred.";
      }
    ...
    switch (name) {
      case "sNftName":
        if (!cleanedValue.trim()) {
          newErrors.sNftName = "NFT Name is required";
        } else if (cleanedValue.trim().length < 1 || cleanedValue.trim().length > 100) {
    ...
    <div className="flex flex-col items-center justify-center p-6 bg-gradient-to-br">
      {isMinting && (
        <div className="fixed inset-0 flex items-center justify-center bg-transparent bg-opacity-50 backdrop-blur-sm z-50">
          <div className="flex flex-col items-center p-8 rounded-lg shadow-2xl animate-pulse">
            ...
            <p className="text-sm text-gray-100 mt-2">This may take a moment.</p>
          </div>
        </div>
      )}
      ...
      <div className="w-[500px] p-8 rounded-xl shadow-2xl bg-gray-950/60 backdrop-blur-lg border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* NFT Name */}
          <div>
            <label htmlFor="sNftName" className="block text-sm font-medium text-gray-200">
              NFT Name
            </label>
            <input
              type="text"
          ...
          {/* NFT Royalty */}
          <div>
            <label htmlFor="nRoyalty" className="block text-sm font-medium text-gray-200">
              NFT Royalty (%)
            </label>
            <input
              type="number"
            ...
            <p className="text-gray-500 text-xs mt-1">Enter royalty percentage (0-100, e.g., 10.5)</p>
          </div>

          {/* Image */}
          <div>
            <label htmlFor="sImageUrl" className="block text-sm font-medium text-gray-300">
              Image
            </label>
            <input
              type="file"
              ...
              className={`w-full text-sm mt-1 text-gray-400
                ...
                hover:file:bg-blue-100 transition duration-300 ${
                  errors.sImageUrl ? "border-red-500 file:bg-red-500 file:text-gray-100 hover:file:text-gray-900" : "border-gray-300"
                }`}
            />
            {errors.sImageUrl && (
          ...
          {/* Description */}
          <div>
            <label htmlFor="nNftDescription" className="block text-sm font-medium text-gray-300">
              Description
            </label>
            <textarea
              id="nNftDescription"
            ...
            {errors.nNftDescription && (
              <p className="text-red-400 text-xs mt-2">{errors.nNftDescription}</p>
            )}
          </div>

          <button
            type="submit"
            disabled={isUploading || isMinting || Object.values(errors).some(Boolean)}  // Disable on errors
          ...
          >
            {(isUploading || isMinting) && <FaSpinner className="animate-spin" />}
