...
const ConnectWallet = () => {
  ...
  const { mutate: mutateConnectWallet, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      const { sWalletAddress, sToken, isVerified, sUsername, sUserProfileImage } = data.data;
      dispatch(setWalletAddress({ sWalletAddress, sToken, isVerified, sUsername, sUserProfileImage }));
      navigate("/home");
  ...
  const handleConnect = async () => {
    try {
      ...
      if (address) {
        ...
        console.log("Triggering wallet connection mutation with address:", sWalletAddress);
